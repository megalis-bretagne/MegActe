...
@Component({
  ...
  imports: [
    ...
    DateInputComponent, SelectInputComponent, CheckboxInputComponent, TextInputComponent, FormsModule, CommonModule
  ],
...
export class ActeFormComponent {
  ...
  assignFileTypes(): void {
    if (!this.isFormFileTypesValid()) {
      console.log("this.isFormFileTypesValid()" + this.isFormFileTypesValid())
      this.modalMessage = 'Veuillez remplir tous les champs requis correctement.';
      console.log("'Veuillez remplir tous les champs requis correctement.'")

      this.isSuccess = false;
      this.isValidTypeForm = false;
      return;
    }
  ...
  isFormFileTypesValid(): boolean {
    const valid = this.selectedTypes.every(type => type !== '' && type !== undefined);
    console.log("isFormFileTypesValid: ", valid, " selectedTypes: ", this.selectedTypes);
    return valid;
  }

  isTypeSelected(index: number): boolean {
    console.log("isTypeSelected() index:", index, " value:", this.selectedTypes[index]);
