...
function createDoc() {
  ...
  router.push(`/org/${props.entiteId}/document/new/edit?type=${selectedFlux.value}`);
}
...
const {
  ...
  isError,
  error,
} = useDocuments(toRef(props, 'entiteId'), selectedFlux, pageActive, search, rowsPerPage);


watch(() => props.entiteId, () => { pageActive.value = 1; search.value = ""; selectedFlux.value = null; });
watch(selectedFlux, () => { pageActive.value = 1; search.value = ""; });
watch(search, () => { searchPage.value = 1; }, { flush: "sync" });
watch(rowsPerPage, () => { searchPage.value = 1; pageActive.value = 1; });
...
async function fetchEtatOptions(fluxType: string) {
  ...
  etatOptions.value = [];
  try {
    const actions = await $fetch<Record<string, any>>(`/flux/${fluxType}/action`, {
...
function openDoc(doc: any) {
  queryClient.setQueryData(['document', doc.id_e, doc.id_d], {
    info: { id_d: doc.id_d, titre: doc.titre, type: doc.type, creation: null, modification: null },
...
function prefetchDoc(doc: any) {
  ...
  queryClient.prefetchQuery({
    ...
    queryFn: () => $fetch(`/entite/${doc.id_e}/document/${doc.id_d}`, {
      baseURL: config.public.apiBaseUrl,
      headers: { Authorization: `Bearer ${user.value?.token}` },
    }),
    staleTime: 30_000,
...
async function duplicateDoc(doc: any) {
  ...
  try {
    ...
    queryClient.invalidateQueries({ queryKey: ['documents', Number(doc.id_e)] });
  } finally {
  ...
  <div class="w-full">
    ...
    >
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="flex flex-col gap-1">
          <label class="text-xs font-medium text-gray-500 uppercase tracking-wide">État</label>
          <Select
            v-model="filterEtat"
            :options="etatOptions"
            :loading="etatLoading"
    ...
    >
      ...
      <Paginator
        ...
        @page="(e) => { rowsPerPage = e.rows; onChangePage(e.page + 1); }"
      />
    </div>
