...
<template>
  ...
  >
    ...
    >
      <button
        v-tooltip.right="collapsed ? 'Déplier le menu' : 'Replier le menu'"
        class="shrink-0 p-1.5 text-gray-500 hover:bg-gray-100 rounded"
        @click="collapsed = !collapsed"
      >
        <AngleRightIcon v-if="collapsed" class="w-4 h-4" />
        <AngleLeftIcon v-else class="w-4 h-4" />
      </button>
    </div>
        ...
        <ul v-else class="space-y-0.5">
          ...
          >
            ...
            <button
              ...
              class="shrink-0 p-1 text-blue-600 hover:text-blue-700"
              @click="createDoc(flux)"
            >
              <i class="pi pi-plus-circle text-lg" />
            </button>
          </li>
        </ul>
      </div>
