...
function downloadSavedFile(filename: string, index: number) {
  ...
  downloadDocumentFile(props.entiteId, props.idD, props.field.key, filename, index);
}
</script>
    ...
    <td class="px-4 py-3">
      ...
      <template v-else-if="field.type === 'file'">
        ...
        <div v-if="pendingFiles?.length" class="mt-1 flex flex-wrap gap-1">
          ...
          >
            {{ f.name }}
            <button type="button" class="hover:text-red-500" @click="emit('fileRemove', f)">×</button>
          </span>
        </div>
      ...
      <template v-else-if="field.type === 'checkbox'">
        <input
            ...
            @change="emit('update:modelValue', ($event.target as HTMLInputElement).checked ? 'checked' : '')"
        />
      </template>
      ...
      <template v-else-if="field.type === 'externalData'">
        <div class="flex items-start gap-3">
          <span v-if="externalDisplayValue" class="text-sm text-gray-700 flex-1 min-w-0 break-words">
