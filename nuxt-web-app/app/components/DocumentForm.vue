...
const FORM_TAB_FIELD_OVERRIDES: Record<string, Record<string, string[]>> = {
  preparer: {
    "deliberations-studio":         ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "arretes-individuels-studio":   ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "actes-reglementaires-studio":  ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "autres-studio":                ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "contrats-studio":              ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "documents-budgetaires-studio": ["acte_nature", "numero_de_lacte", "objet", "comment", "arrete", "autre_document_attache", "publication_open_data"],
    "autres-studio-sans-tdt":       ["acte_nature", "nature_autre_detail", "numero_de_lacte", "objet", "date_de_lacte", "classification", "arrete", "autre_document_attache", "publication_open_data"],
  },
  acte: {
    default: ["date_de_lacte", "document_papier", "classification", "type_piece"],
  },
};
...
const formData = ref<Record<string, any>>(doc.value?.data ? { ...doc.value.data } : {});
...
function parseAcceptFromComment(comment: string | null): string | null {
  ...
  const matches = comment.match(/\b(PDF|DOC|DOCX|ODT|XLS|XLSX|ODS|PNG|JPG|JPEG|GIF|ZIP|XML|CSV|TXT|RTF|ODP|PPT|PPTX)\b/gi);
...
async function openExternalDialog(fieldKey: string, fieldLabel: string) {
  ...
  if (fieldKey === "type_piece") {
    ...
    try {
      ...
      if (status === 400) {
        ...
        saveError.value = e?.data?.detail ?? e?.message ?? "Impossible de charger les types de pièces";
      }
    } finally {
  ...
  if (!externalDataCache.value[fieldKey]) {
    ...
    try {
      ...
      saveError.value = e?.data?.detail ?? e?.message ?? `Impossible de charger les options pour « ${fieldLabel} »`;
      externalDataLoading.value = null;
      return;
    } finally {
      externalDataLoading.value = null;
...
const { mutateAsync: saveDoc, isPending: saving } = useMutation({
  mutationFn: async () => {
    ...
    for (const [key, files] of Object.entries(pendingFiles.value)) {
      ...
      const uploadRes = await fetch(uploadUrl, { method: "POST", headers: authHeaders.value, body: form });
      if (!uploadRes.ok) {
  ...
  for (const tab of tabs.value) {
    for (const key of tab.fields) {
      ...
      const isEmpty = def.type === "file"
        ...
        : !formData.value[key] || (Array.isArray(formData.value[key]) && !formData.value[key].length) || formData.value[key] === "";
      if (isEmpty) {
...
<template>
  <div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <template v-else>
      <div v-if="saveError" class="mb-4 text-sm text-red-700 bg-red-50 px-4 py-3 rounded border border-red-300">
        {{ saveError }}
      </div>

      <h1 class="text-2xl font-bold text-gray-900 mb-6">
        {{ isNew ? "Nouveau document" : (doc?.info?.titre ?? "Éditer le document") }}
      </h1>
            ...
            <tr v-else class="even:bg-gray-50">
              ...
              <td class="px-4 py-3">
                ...
                <template v-else-if="field!.type === 'file'">
                  <label class="inline-flex items-center gap-2 px-3 py-1 text-sm border border-gray-300 rounded bg-white hover:bg-gray-50 cursor-pointer w-fit">
                    <span>Choisir {{ field!.multiple ? 'des fichiers' : 'un fichier' }}</span>
                    <input
                  ...
                  <div v-if="pendingFiles[field!.key]?.length" class="mt-1 flex flex-wrap gap-1">
                    <span
                    ...
                    >
                      {{ f.name }}
                      <button type="button" class="hover:text-red-500" @click="removeFile(field!.key, f)">×</button>
                    </span>
                  </div>
                  <p v-else-if="formData[field!.key]?.length" class="mt-1 text-xs text-gray-400">
                    Actuel :
                ...
                <template v-else-if="field!.type === 'checkbox'">
                  <input
                    type="checkbox"
                    :checked="formData[field!.key] === 'checked' || formData[field!.key] === '1'"
                    :disabled="field!.readonly"
                    class="w-4 h-4 accent-blue-600 disabled:cursor-not-allowed disabled:opacity-60"
                    @change="formData[field!.key] = ($event.target as HTMLInputElement).checked ? 'checked' : ''"
                  />
                </template>
    ...
    >
      ...
      <div class="max-h-96 overflow-y-auto divide-y divide-gray-100 border border-gray-200 rounded">
        ...
        <template v-for="opt in filteredExternalOptions" :key="opt">
          ...
          >
            <i v-if="externalDialogTemp.includes(opt)" class="pi pi-check text-blue-600" />
            {{ opt }}
          </button>
        </template>
      </div>
    ...
    >
      ...
      <template #footer>
        <Button label="Annuler" severity="secondary" @click="showTypePieceDialog = false" />
        <Button
          label="Valider"
