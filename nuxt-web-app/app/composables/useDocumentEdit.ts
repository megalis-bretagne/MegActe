    ...
    async function openExternalDialog(fieldKey: string, fieldLabel: string, onError?: (msg: string) => void) {
        if (fieldKey === "type_piece") {
            ...
            try {
                ...
                if (status === 400) {
                    ...
                    onError?.(e?.data?.detail ?? e?.message ?? "Impossible de charger les types de pièces");
                }
            } finally {
        ...
        if (!externalDataCache.value[fieldKey]) {
            ...
            try {
                ...
                onError?.(e?.data?.detail ?? e?.message ?? `Impossible de charger les options pour « ${fieldLabel} »`);
                externalDataLoading.value = null;
                return;
            } finally {
                externalDataLoading.value = null;
