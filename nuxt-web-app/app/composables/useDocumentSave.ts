...
export function useDocumentSave(
  ...
  async function ensureDocExists(): Promise<string> {
    ...
    const response = await apiFetch<DocumentDetail>(
  ...
  const { mutateAsync: saveDoc, isPending: saving } = useMutation({
    mutationFn: async (opts: { phase: SavePhase }) => {
      ...
      const wantsTypePieceMeta =
        opts.phase === "step1" && state.hasStepTdt.value;
      ...
      if (opts.phase === "step1" || opts.phase === "single-send") {
        ...
        for (const tab of state.tabs.value) {
          for (const key of tab.fields) {
            ...
            if (state.formData.value[key] !== undefined)
              textData[key] = state.formData.value[key];
          }
        }

        const patchPromise = apiFetch(
          `/entite/${props.entiteId}/document/${idD}`,
          {
            method: "PATCH",
        ...
        const uploadPromises = Object.entries(state.pendingFiles.value)
          ...
          .map(async ([key, files]) => {
            ...
            try {
              await apiFetch(
                `/entite/${props.entiteId}/document/${idD}/file/${key}`,
                {
                  method: "POST",
              ...
              throw new Error(
                err?.data?.detail ??
                  `Erreur lors de l'upload du fichier (${err?.status ?? err?.response?.status ?? "?"})`
              );
            }

            // Vide les fichiers en attente une fois uploadés (et reflète leur nom dans
            // formData) : resauvegarder sans ça les renverrait 2x, doublons Pastell à l'étape 2.
            const names = files.map((f) => f.name);
            state.formData.value[key] = isMultiple
              ? [
                  ...(Array.isArray(state.formData.value[key])
                    ? state.formData.value[key]
                    : []),
                  ...names,
                ]
              : names[0];
      ...
      if (opts.phase === "step2-send" || opts.phase === "single-send") {
        try {
          ...
          throw new Error(
            `Document enregistré, mais l'envoi a échoué${detail ? ` : ${detail}` : ""}.`
          );
        }
      }

      // Flux TDT : bascule vers l'étape 2 après l'étape 1. Nécessite le fichier acte déjà
      // présent côté Pastell, donc appelé après les uploads ci-dessus, pas en parallèle.
  ...
  function validateBeforeSave(): boolean {
    ...
    const numeroActeError = validateNumeroActe(
      state.formData.value.numero_de_lacte
    );
    if (numeroActeError) {
    ...
    for (const tab of state.tabs.value) {
      for (const key of tab.fields) {
        ...
        const isEmpty =
          def.type === "file"
            ? !state.pendingFiles.value[key]?.length &&
              !state.formData.value[key]
            : !state.formData.value[key] ||
              (Array.isArray(state.formData.value[key]) &&
                !state.formData.value[key].length) ||
              state.formData.value[key] === "";
  ...
  function finalizeTypePiece(): boolean {
    ...
    if (state.typePieceItems.value.some((item) => !item.label)) {
      saveError.value =
        "Veuillez sélectionner tous les types de fichiers requis.";
      return false;
    }
    state.formData.value.type_piece = state.typePieceItems.value.map(
      (item) => item.label
    );
    return true;
  }
  ...
  async function save() {
    // Rien n'a changé depuis l'étape 2 (Précédent puis Étape suivante sans modif) : évite
    // un PATCH/upload inutile et préserve les sélections de classification déjà faites.
  ...
  return {
    ...
    savingTypePiece: computed(
      () => saving.value && activePhase.value === "step2-save"
    ),
    sending: computed(
      () =>
        saving.value &&
        (activePhase.value === "step2-send" ||
          activePhase.value === "single-send")
    ),
  };
}
