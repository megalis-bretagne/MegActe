...
def version():
    ressource = "/version"
    PArequest = requests.get(
        root_url + ressource, auth=HTTPBasicAuth(u, p), timeout=2000
    )
...
def user():
            ...
            for index, usr in enumerate(users):
                ...
                if usr["login"] == username:
                    ...
                    request2 = requests.get(
                        root_url + ressource2, auth=HTTPBasicAuth(u, p), timeout=2000
                    )
    ...
    if g.uid in PASTELL_SESSIONS.keys():
        ...
        logger.info(
            "%s - %s - entité : %s - %s" % (g.user, allowed_role, id_e, "Connecté")
        )
...
@current_app.route(
    "/document/<string:id_doc>/action/<string:id_action>", methods=["POST"]
)
@login_required
    ...
    if request.method == "GET":
        ...
        if status == 201:
            ...
            return json_response(
                {
                    "pastel": data,
                    "link": "%s/Document/detail?id_d=%s&id_e=%s"
                    % (root_url.split("/back/")[0], data["id_d"], IDE),
                },
                status,
            )
        else:
        ...
        if status == 200:
            data = json.loads(PA_request.text)
            logger.info(
                "%s - %s - %s - %s" % (USER, "Document modifié", id_doc, params)
            )
...
@current_app.route(
    "/document/<string:id_doc>/file/<string:element>/<string:numero>",
    methods=["POST", "DELETE"],
)
@login_required
def addFile(id_doc, element, numero="0"):
    ...
    ressource = "/entite/%s/document/%s/file/%s/%s" % (
        PASTELL_SESSIONS[g.uid]["details"]["id_e"],
        id_doc,
        element,
        numero,
    )
    if request.method == "POST" and id_doc and element:
        ...
        if "file_content" in request.files:
            ...
            if status == 201:
                data = json.loads(PA_request.text)
                logger.info(
                    "%s - %s - %s - %s" % (USER, id_doc, element, "Fichier publié")
                )
                return json_response({"pastel": data}, status)
            else:
                return json_response({"status": "error"}, status)
        ...
        if status == 200:
            ...
            logger.info(
                "%s - %s - %s - %s" % (USER, id_doc, element, "Fichier supprimé")
            )
...
@current_app.route(
    "/document/<string:id_doc>/externalData/<string:field>", methods=["GET"]
)
@login_required
def externalData(id_doc, field):
    # ExternaData
    ressource = "/entite/%s/document/%s/externalData/%s" % (
        PASTELL_SESSIONS[g.uid]["details"]["id_e"],
        id_doc,
        field,
    )
