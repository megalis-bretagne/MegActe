...
class ApiS2low:
    ...
    def get_url_post_confirm(
        ...
        hash = auth.password  # todo hast
        url = f"{self._config.base_url}{self.ACTES_TRANSAC_POST_CONFIRME}?id={tedetis_transaction_id}&nounce={nounce}&login={auth.username}&hash={hash}"
        return url
