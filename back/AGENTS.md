...
- Authorization flow: every router except `health` is protected by `dependencies.validate_token` (validates a Keycloak JWT against Keycloak's JWKS, checked in `app/main.py` via `Depends`). `get_current_user` then extracts the `preferred_username` from the token, and `database.get_user_from_db` looks that login up in the local `pastell_users` table to get the user's **Pastell** token (stored encrypted, only token used for Pastell; see `models/users.py::UserPastell.get_decrypt_token` / `app/utils`). The admin technical account also authenticates with a Bearer token (`settings.pastell.token`); the only Basic Auth with a user's credentials is the token-creation call at enrollment (`UserService.create_user_token`).
